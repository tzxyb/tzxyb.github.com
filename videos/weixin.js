 ...
 * 组装原链接带过来的参数
 */
function pingjieParams(shareurl,code){
	...
	if(code && code=='coupon_couponmmc'){
		var rid = localgetUrlParam("rid")||$("#rid").val();
		var buyer = localgetUrlParam("buyer")||$("#buyername").val();
		var store = $("#store").val()||localgetUrlParam("store");
		var car = localgetUrlParam("car")||$("#vehicletype").val();
		if(store.lastIndexOf('店')==-1){
			store += "店";
		}
		shareurl = shareurl.replace(/\$rid/g,rid);
		shareurl = shareurl.replace(/\$buyer/g,buyer);
		shareurl = shareurl.replace(/\$store/g,store);
		shareurl = shareurl.replace(/\$car/g,car);
		
		var strs = shareurl.split(/back_url\=/g);
		if(strs.length==2){
			shareurl = strs[0]+"back_url="+encodeURIComponent(strs[1]);
		}
	}
	return shareurl;
}
 ...
 * 处理分享标题
 */
function handleShareTitle(title,code){
	if(code && code=='coupon_couponmmc'){
		var buyer = localgetUrlParam("buyer")||$("#buyername").val();
		title = title.replace(/\$buyer/g,buyer);
	}
	return title;
}
 ...
 * 处理摘要
 */
function handleShareWXContent(wxcontent,code){
	if(code && code.indexOf("szcs8_")>-1){//蔡司眼镜
		var num = $("#endTime").val();
		wxcontent = wxcontent.replace(/\$num/g,num);
	}
	return wxcontent;
}
 ...
 * 处理朋友圈标题
 */
function handlePYQContent(pyqContent,code){
	if(code && code=='coupon_couponmmc'){
		var store = $("#store").val()||localgetUrlParam("store");
		var car = localgetUrlParam("car")||$("#vehicletype").val();
		if(store.lastIndexOf('店')==-1){
		...
		pyqContent = pyqContent.replace(/\$store/g,store);
		pyqContent = pyqContent.replace(/\$car/g,car);
	}else if(code && code.indexOf("szcs8_")>-1){//蔡司眼镜
		var num = $("#endTime").val();
		var nickname = localgetUrlParam("nickname");
		pyqContent = pyqContent.replace(/\$num/g,num);
		pyqContent = pyqContent.replace(/\$nickname/g,nickname);
	}
	return pyqContent;
}
...
function getWeixinShareData(){
    ...
    var params = {};
    params.url = url;
    if(location.href.indexOf("/template_2") > 0 || location.href.indexOf("/template_3") > 0){
    	params.origin = from;
    }else if(origin){
    	params.origin = origin;
    }
    var openId = $("#openId").val()||localgetUrlParam("openId");
    if(openId){
    ...
    if(window.location.href.indexOf("/mmcyy/result") > 0){
    	num= localgetUrlParam("result");
    }
    if(num){
    	params.num = num;
    }
        ...
        success: function (data) {
            if(data && data.status === 0 && data.result){
            	...
            	if(data.result.shareTitle){
            		shareTitle = handleShareTitle(data.result.shareTitle,params.origin);
            	}
            	if(data.result.shareWXContent){
            		shareWXContent = handleShareWXContent(data.result.shareWXContent,params.origin);
            	}
                if(data.result.url){
                	var shareurl = data.result.url;
                	hrefUrl = pingjieParams(shareurl,params.origin);
                }
                if(data.result.sharePYQContent){
                	pyqContent = handlePYQContent(data.result.sharePYQContent,params.origin);
            	}
