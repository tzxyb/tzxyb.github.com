...
var gDomain="sdc.zuche.com";	// SDC Production Mode Domain
var gDcsId="dcswwefp210000oej7isqjplt_1q7t";
var gFpc="WT_FPC";
var gConvert=true;
...
function dcsOther(){
	...
	if (typeof(WT.dcsvid)!="undefined") {
		...
		var vid1=dcsGetCookie(gVisitorId);
		if (vid1!==null) {
			vid1=vid1.replace(/(^\s*)|(\s*$)/g, "").toLowerCase();
			if (vid1!==""&&vid1!="null") {
				WT.dcsvid=escape(vid1);
			}
		} 
	}
...
function dcsVar(){
	...
	if (window.ActiveXObject){
		...
		for (var i1=0;i1<navigator.plugins.length;i1++){
			if (navigator.plugins[i1].name.indexOf('Shockwave Flash')!=-1){
				WT.fi="Yes";
				WT.fv=navigator.plugins[i1].description.split(" ")[2];
				break;
			}
		}
	}
...
//	WT.sp="@@SPLITVALUE@@";
	...
	if (window.location.search){
		...
		if (gQP.length>0){
			for (var i2=0;i2<gQP.length;i2++){
				var pos=DCS.dcsqry.indexOf(gQP[i2]);
				if (pos!=-1){
					var front=DCS.dcsqry.substring(0,pos);
					var end=DCS.dcsqry.substring(pos+gQP[i2].length,DCS.dcsqry.length);
		...
		for (var i=0;i<length;i++){
			...
			if (name.length>0){
				if (name.indexOf("WT.")===0){
					...
					var encode1=false;
					if (gI18n){
						var params1=["wt_visitor_id"];
						for (var j1=0;j1<params1.length;j1++){
							if (name.indexOf("DCSext."+params1[j1])===0){
								encode1=true;
								break;
							}
						}
					}
					DCSext[name.substring(7)]=encode1?dcsEscape(dcsEncode(content),I18NRE):content;
				}
...
function dcsTag(){
	...
	{
	    DCS.dcsref = DCS.dcsref.substring(0,900);
	}
